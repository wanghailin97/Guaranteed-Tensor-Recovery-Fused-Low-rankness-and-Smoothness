...
rng(1);
%% initial Data
methodname  = {'IALM-RPCA','HoRPCA','t-SVD','KBR-RPCA','TRPCA-CTV','TRPCA-WCTV'};
...
if EN_TRPCA_CTV
    opts = [];
    opts.directions = [1 2];
    lambda = 1/sqrt(min(sizeD(1),sizeD(2))*sizeD(3));
    tic;
    [B{i}, F{i}] =   TRPCA_CTV(D,lambda,opts);
    Time(i) = toc;
    [fmeasure(i), S{i}, bestTH(i)] = findFMeasure(F{i}(:,:,testNum), S0);
    disp([methodname{i}, ' done in ' num2str(Time(i)), ' s.'])
    disp([methodname{i}, ' Fmeasure ' num2str(fmeasure(i)), ' .'])
...
if EN_TRPCA_WCTV
    ...
    lambda = 1/sqrt(min(sizeD(1),sizeD(2))*sizeD(3));
    tic;
    [B{i}, F{i}] =   TRPCA_WCTV(D,lambda,opts);
    Time(i) = toc;
    [fmeasure(i), S{i}, bestTH(i)] = findFMeasure(F{i}(:,:,testNum), S0);
    disp([methodname{i}, ' done in ' num2str(Time(i)), ' s.'])
    disp([methodname{i}, ' Fmeasure ' num2str(fmeasure(i)), ' .'])
