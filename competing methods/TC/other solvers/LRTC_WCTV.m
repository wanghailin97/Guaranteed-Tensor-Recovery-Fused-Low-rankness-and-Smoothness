%-----Low-tubal-rank Tensor Completion via Wighted 3D Correlted Total Viariation-----%
function [X,obj,err,iter] =LRTC_WCTV(M,Omega,opts) 
...
while iter<max_iter
    ...
    X = real( ifftn( fftn( mu*(M-E)+M4+H )./(mu*(1+T)) ) );
  
    %% Updata G1 G2 G3 -- proximal operator of TNN
    [G1,tnn_G1] = prox_tnn(diff_1(X)+M1/mu,weights(1)/mu);
    [G2,tnn_G2] = prox_tnn(diff_2(X)+M2/mu,weights(2)/mu);
    [G3,tnn_G3] = prox_tnn(diff_3(X)+M3/mu,weights(3)/mu);
    ...
    if detail
        if iter == 1 || mod(iter, 10) == 0
            obj = weights(1)*tnn_G1+weights(1)*tnn_G2+weights(1)*tnn_G3;
            err = norm(dY(:),'fro');
