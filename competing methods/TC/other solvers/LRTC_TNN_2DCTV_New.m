...
function [X,obj,err,iter] =LRTC_TNN_2DCTV_New(M,Omega,opts) 
...
while iter<max_iter
    ...
    H = diff_1T_new(mu*G1-M1)+diff_2T_new(mu*G2-M2);
    X = real( ifftn( fftn( mu*(M-E)+M3+H )./(mu*(1+T)) ) );
  
    %% Updata G1 G2-- proximal operator of TNN
    [G1,tnn_G1] = prox_tnn(diff_1_new(X)+M1/mu,1/mu);
    [G2,tnn_G2] = prox_tnn(diff_2_new(X)+M2/mu,1/mu);
    ...
    %% Update M1 M2 M3 mu
    M1 = M1+mu*(diff_1_new(X)-G1);
    M2 = M2+mu*(diff_2_new(X)-G2);
    M3 = M3+mu*(M-X-E);
