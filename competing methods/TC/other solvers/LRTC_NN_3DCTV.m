%-----Low-tubal-rank Tensor Completion via 3D Correlted Total Viariation-----%
function [X,obj,err,iter] =LRTC_NN_3DCTV(M,Omega,opts) 
...
while iter<max_iter
    ...
    X = real( ifftn( fftn( mu*(M-E)+M4+H )./(mu*(1+T)) ) );
     ...
     %% Updata G1 G2 G3-- proximal operator of NN
    ...
    [G3M,nn_G3] = prox_nuclear(reshape(diff_3(X)+M3/mu,[h*w,d]),1/mu);
     G3         = reshape(G3M,[h,w,d]);
