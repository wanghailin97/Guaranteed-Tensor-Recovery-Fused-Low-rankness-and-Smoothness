...
function [X,obj,err,iter] =LRTC_NN_2DCTV(M,Omega,opts) 
...
%     [u,s,v]= svd(reshape(diff_1(X),[h*w,d]),'econ');
%     G1M    = u*diag(prox_l1(diag(s),1/mu))*v';
%     G1     = reshape(G1M,[h,w,d]);
%     [u,s,v]= svd(reshape(diff_2(X),[h*w,d]),'econ');
%     G2M    = u*diag(prox_l1(diag(s),1/mu))*v';
%     G2     = reshape(G2M,[h,w,d]);
    ...
    chgE = max(abs(Ek(:)-E(:)));
    chg  = max([chgX chgE max(abs(dY(:))) dif]);
    if chg < tol
        break;
    end 
