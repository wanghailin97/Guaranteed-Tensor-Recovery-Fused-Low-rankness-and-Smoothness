%-----Low-tubal-rank Tensor Completion via TNN plus 2D Total Viariation-----%
function [X,obj,err,iter] =LRTC_TNN_2DTV_new(M,Omega,lambda,opts) 
...
while iter<max_iter
    ...
    X = real( ifftn( fftn( mu*(M+Y-E)+M3+M4+H )./(mu*(2+T)) ) );
  
    %% Update Y -- proximal operator of TNN
    [Y,tnn_Y] = prox_tnn(X-M3/mu,1/mu);
    
    %% Updata G1 G2 -- soft-thresholding operator 
    G1 = prox_l1(diff_1(X)+M1/mu,lambda/mu);
    G2 = prox_l1(diff_2(X)+M2/mu,lambda/mu);
