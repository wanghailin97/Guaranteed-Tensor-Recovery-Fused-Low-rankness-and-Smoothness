...
X_BCPF = double(model.X);
imshow(X_BCPF)
RSElist(1,1) = perfscore(X_BCPF, X);
RSElist(1,2) = perfscore(X_BCPF(O==1), X(O==1));
RSElist(1,3) = perfscore(X_BCPF(O==0), X(O==0));

X_BCPF(O==1) = X(O==1);
PSNRlist(1) = PSNR_RGB(X_BCPF,X);
SSIMlist(1) = ssim_index(rgb2gray(uint8(X_BCPF)),rgb2gray(uint8(X)));
